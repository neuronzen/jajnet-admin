...
function init(name){return (name||'U').trim().charAt(0).toUpperCase()}
...
async function doLogin(){
  ...
  try{
    ...
    const msg=err.code==='auth/not-admin'?'এই অ্যাকাউন্টটি অ্যাডমিন নয়':mapAuthError(err.code);
    showErr(msg);
  }finally{
...
auth.onAuthStateChanged(user=>{
  ...
  if(user && ADMIN_EMAILS.includes(user.email)){
    ...
    if(!user) setTimeout(()=>{try{$('#emailInput').focus()}catch(e){}},300);
    stopApp();
  }
});
...
document.addEventListener('click',e=>{
  ...
  if(nav){e.preventDefault();goto(nav.dataset.page);return}
});
...
function startApp(){
  ...
  unsubs.push(db.collection('notices').onSnapshot(()=>{
...
async function renderDashboard(){
  ...
  try{
    const [uS,pS]=await Promise.all([db.collection('users').get(),db.collection('payments').get()]);
    const users=uS.docs,pays=pS.docs;
    let active=0,due=0,expired=0,dueAmt=0;
    users.forEach(d=>{
      const m=d.data(),s=m.status||'';
      if(s==='active')active++;
      else if(s==='due')due++;
...
<div class="hero-date">${dateStr}</div>
        ...
        <div class="hero-icon">
<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"><path d="M5 12.55a11 11 0 0 1 14.08 0"/><path d="M1.42 9a16 16 0 0 1 21.16 0"/><path d="M8.53 16.11a6 6 0 0 1 6.95 0"/><line x1="12" y1="20" x2="12.01" y2="20"/></svg>
        </div>
      </div>
...
const m=d.data(),s=(m.status||'').toLowerCase();
...
return `<div class="list-row">
  ...
  <div class="list-main">
    ...
    <div class="list-sub">${esc(m.package||'')} • ৳${m.packagePrice||0}</div>
  </div>
...
// ============ CUSTOMERS ============
let custSearch='',custFilter='all',custCache=[];
async function renderCustomers(){
  ...
  try{
    ...
    body.innerHTML='<div class="empty">লোড করা যায়নি</div>';
  }
  body.dataset.loading='0';
}
function paintCustomers(){
  ...
  if(custSearch){
    ...
    list=list.filter(m=>(m.name||'').toLowerCase().includes(q)||(m.phone||'').includes(q)||(m.email||'').toLowerCase().includes(q));
  }
  ...
  body.innerHTML=`
    <div class="toolbar">
      <div class="search">
        ...
        <input id="custSearchInput" placeholder="খুঁজুন..." value="${esc(custSearch)}">
      </div>
    ...
    <div id="custList">
      ${list.length?list.map(customerRow).join(''):'<div class="empty">কোনো গ্রাহক নেই</div>'}
    </div>`;
  const si=$('#custSearchInput');
  si.oninput=e=>{custSearch=e.target.value;const p=si.selectionStart;paintCustomers();const ns=$('#custSearchInput');ns.focus();ns.setSelectionRange(p,p)};
  $('#addCustBtn').onclick=openAddCustomer;
  $$('.chips .chip').forEach(c=>c.onclick=()=>{custFilter=c.dataset.f;paintCustomers()});
...
function openAddCustomer(){
  openModal(`
    ...
    <div class="form-row"><label>প্যাকেজ</label><select id="acPkg">${Object.entries(PACKAGES).map(([k,v])=>`<option value="${k}">${k} — ৳${v}</option>`).join('')}</select></div>
  ...
  $('#acPkg').onchange=()=>{const v=PACKAGES[$('#acPkg').value];$('#acPrice').value=v;$('#acDue').value=v};
...
async function submitAddCustomer(){
  ...
  const btn=$('#acSave');btn.disabled=true;btn.textContent='অপেক্ষা করুন...';
  try{
    let secondary=firebase.apps.find(a=>a.name==='admin_creator');
    ...
    await db.collection('users').doc(cred.user.uid).set({
      name,phone:$('#acPhone').value.trim(),email,
...
function openCustomerDetail(id){
  ...
  openModal(`
    ...
    <div class="form-row"><label>প্যাকেজ</label><select id="dcPkg">${Object.keys(PACKAGES).map(k=>`<option value="${k}" ${k===m.package?'selected':''}>${k} — ৳${PACKAGES[k]}</option>`).join('')}</select></div>
    <div class="form-grid-2">
    ...
    <div class="form-row"><label>স্ট্যাটাস</label><select id="dcStatus">${['active','due','expired'].map(s=>`<option value="${s}" ${s===m.status?'selected':''}>${s[0].toUpperCase()+s.slice(1)}</option>`).join('')}</select></div>
    <div class="modal-actions">
...
async function renderPayments(){
  ...
  try{
    ...
    paintPayments();
  }catch(e){body.innerHTML='<div class="empty">লোড করা যায়নি</div>'}
  body.dataset.loading='0';
}
function paintPayments(){
  const body=$('#pageBody');
  const list=payTab==='pending'?payCache.filter(p=>p.status==='pending'):payCache;
  body.innerHTML=`
    <div class="chips">
  ...
  $$('#payList .pay-verify').forEach(b=>b.onclick=()=>verifyPayment(b.dataset.id));
  $$('#payList .pay-reject').forEach(b=>b.onclick=()=>rejectPayment(b.dataset.id));
}
function paymentCard(p){
  const s=p.status||'pending';
  const cls=s==='verified'?'pill-success':s==='pending'?'pill-warning':'pill-error';
  const ic=s==='verified'?'&#10003;':s==='pending'?'&#8987;':'&#10005;';
...
async function verifyPayment(id){
  try{await db.collection('payments').doc(id).update({status:'verified',verifiedAt:firebase.firestore.FieldValue.serverTimestamp()});toast('পেমেন্ট Verify হয়েছে ✅','success')}
  catch(e){toast('সমস্যা হয়েছে','error')}
}
async function rejectPayment(id){
  try{await db.collection('payments').doc(id).update({status:'rejected'});toast('পেমেন্ট বাতিল করা হয়েছে','error')}
  catch(e){toast('সমস্যা হয়েছে','error')}
}
  ...
  return `<div class="card">
    <div style="display:flex;gap:12px;align-items:flex-start">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:13px;color:var(--grey);word-break:break-word">${esc(n.body||'')}</div>
      </div>
      <button class="btn btn-danger notice-del" data-id="${n.id}" style="padding:6px 10px;font-size:12px;flex-shrink:0">মুছুন</button>
    </div>
  </div>`;
}
function openAddNotice(){
  ...
  $('#anSave').onclick=async()=>{
    ...
    try{
      await db.collection('notices').add({title:t,body:b,createdAt:firebase.firestore.FieldValue.serverTimestamp()});
      closeModal();
      toast('নোটিশ পাঠানো হয়েছে ✅','success');
      renderNotices();
    }catch(e){toast('পাঠানো যায়নি','error');btn.disabled=false;btn.textContent='পাঠান'}
  };
}
async function deleteNotice(id){
  if(!confirm('নোটিশ মুছবেন?'))return;
  try{await db.collection('notices').doc(id).delete();toast('নোটিশ মুছে ফেলা হয়েছে','success');renderNotices()}
  catch(e){toast('সমস্যা হয়েছে','error')}
}
